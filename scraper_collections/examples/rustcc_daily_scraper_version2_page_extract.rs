...
async fn get_page(basic_url: &str, client: &Client, section_id: &str, page: &dyn ToString) -> Result<Document, reqwest::Error> {
    ...
    println!("{}", page_url);
    let res = client.get(page_url).headers(headers).send().await?;
...
async fn main() -> Result<(), reqwest::Error> {
    ...
    let base_url = "https://rustcc.cn/section";
    let section_id = "f4703117-7e6b-4caf-aa22-a3ad3db6898f";
    let mut page = 1;
    while page < 60 {
        let document = get_page(base_url, &client, section_id, &page).await.unwrap();
